...
def generate_report(test_results, component_results, total_time, categories):
    ...
    with open(report_file, 'w') as f:
        ...
        f.write(f"Average Tokens Generated: {statistics.mean([r['tokens_generated'] for r in test_results]):.1f}\n")
        f.write(f"Total Tool Calls: {sum([r['tool_calls'] for r in test_results])}\n\n")
        ...
        for result in test_results:
            ...
            category_stats[category]['tokens'].append(result['tokens_generated'])
        ...
        for result in test_results:
            ...
            f.write(f"  Tokens: {result['tokens_generated']}\n")
...
def main():
    ...
    print(f"Average Tokens: {statistics.mean([r['tokens_generated'] for r in test_results]):.1f}")
    print(f"Total Tool Calls: {sum([r['tool_calls'] for r in test_results])}")
